...
proc uboot_memory_config args {
	...
	if {[string equal -nocase $cpu_arch "microblaze"] == 1} {
		...
		uboot_conf_define $fid CONFIG_SYS_GBL_DATA_OFFSET "([dict get $kconfig_dict memory size] - GENERATED_GBL_DATA_SIZE)"
		set size "0x100000"
	...
	puts $fid "\n/* Size of malloc() pool */"
	uboot_conf_define $fid SIZE $size
	uboot_conf_define $fid CONFIG_SYS_MALLOC_LEN SIZE
	if {[string equal -nocase $cpu_arch "microblaze"] == 1} {
		uboot_conf_define $fid CONFIG_SYS_MONITOR_LEN SIZE
		uboot_conf_define $fid CONFIG_SYS_MONITOR_BASE "([dict get $kconfig_dict memory baseaddr] + CONFIG_SYS_GBL_DATA_OFFSET - CONFIG_SYS_MONITOR_LEN - GENERATED_BD_INFO_SIZE)"
		uboot_conf_define $fid CONFIG_SYS_MALLOC_BASE "(CONFIG_SYS_MONITOR_BASE - CONFIG_SYS_MALLOC_LEN)"
		puts $fid "\n/* stack */"
...
proc uboot_bootenv {fid kconfig_fid} {
	...
	append data "\n" "	\"default_bootcmd=${default_bootcmd}\\0\"" { \ }
	append data "\n" "\"\""
	append data "\n" ${uboot_cmd_data}
	puts $fid $data
}
...
proc gen_axi_qspi_clk args {
	...
	if {[hsi get_property CONFIG.C_XIP_MODE [hsi get_cells $ip]] == 1} {
		set def_clk_pin "S_AXI_ACLK"
	} elseif {[hsi get_property CONFIG.C_TYPE_OF_AXI4_INTERFACE [hsi get_cells $ip]] == 1} {
		set def_clk_pin "S_AXI4_ACLK"
	}
	set bus_freq [hsi::utils::get_clk_pin_freq [hsi get_cells $ip] $def_clk_pin]
...
proc gen_timer_define args {
	...
	set intr_pin_name [hsi get_pins -of_objects [hsi get_cells $ip] -filter {TYPE==INTERRUPT}]
	...
	set timer_freq [hsi::utils::get_clk_pin_freq [hsi get_cells $ip] "S_AXI_ACLK"]
...
proc gen_uartns_define args {
	set fid [lindex $args 0]
	set ip [hsi get_cells [lindex $args 2]]
...
proc get_dma_slave {fid db_dict ip main_key config_cat config_var} {
	...
	set slave_list [hsi get_property "SLAVES" [hsi get_cells $ip]]
	foreach slave $slave_list {
		set ip_type [hsi get_property "IP_NAME" [hsi get_cells $slave]]
		if {[regexp -all {axi_dma} $ip_type ] == 1 } {
			set dma_slaves [hsi get_property "SLAVES" [hsi get_cells $slave]]
	...
	set ip_list [hsi get_cells]
	foreach ip $ip_list {
		set ip_type [hsi get_property "IP_NAME" [hsi get_cells $ip]]
		if {[regexp -all {axi_dma} $ip_type ] == 1 } {
			set tconf_list [get_conf_list $db_dict "axi_dma"]
			foreach conf_cat $tconf_list {
...
{ 4800, 9600, 19200, 38400, 57600, 115200 }
	...
	if { [dict exists $kconfig_dict subsys_conf uboot_config_petalinux]} {
		uboot_set_kconfig_value $kconfig_fid SYS_CONFIG_NAME "\\\"platform-top\\\""
	} else {
		uboot_set_kconfig_value $kconfig_fid SYS_CONFIG_NAME 0 1
	}
	uboot_set_kconfig_value $kconfig_fid SPL "n"
	uboot_set_kconfig_value $kconfig_fid CMD_EEPROM "n"
...
proc uboot_config_mk_gen {fid kconfig_fid} {
	...
	if {[string equal -nocase $cpu_arch "microblaze"] == 1} {
		...
		# get cpu version
		set vlnv [hsi get_property "VLNV" [hsi get_cells $target_cpu]]
...
proc uboot_kconfig_disable {fid} {
	set opt_list {}
	set mapping_list {serial_dict timer_dict reset_gpio_dict
			  flash_dict ethernet_dict sd_dict rtc_dict
			  sata_dict i2c_dict usb_dict processor_dict timer_dict}
	global serial_dict timer_dict reset_gpio_dict processor_dict
